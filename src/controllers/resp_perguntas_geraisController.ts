...
export const resp_perguntas_gerais = async (req: FastifyRequest, reply: FastifyReply) => {

    try {
        // Buscando os dados do banco de dados
        const dados = await db('Perguntas_gerais').select('fk_Usuario_id_usuario', 'nome', 'sexo', 'idade', 'peso', 'estatura');
        ...
        const resp_perguntas_gerais = (dados as any[]).map(user => {
            ...
            console.log(`Calculando IMC para o usuário ${user.nome} - Peso: ${user.peso}, Estatura: ${user.estatura}, IMC: ${imc.toFixed(2)}`); // Log do cálculo do IMC
            return {
