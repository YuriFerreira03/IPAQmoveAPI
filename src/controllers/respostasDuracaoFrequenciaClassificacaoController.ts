...
let CaminhadaFrequenciaTotal = (Q1B: number, Q2E: number, Q4A: number): number => {
    return (Q1B + Q2E + Q4A);
};

let ModeradaDuracaoTotal = (Q1E: number, Q2D: number, Q3B: number, Q3D: number, Q4D: number): number => {
    return (Q1E + Q2D + Q3B + Q3D + Q4D);
};

let ModeradaFrequenciaTotal = (Q1D: number, Q2C: number, Q3A: number, Q3C: number, Q4C: number): number => {
    return (Q1D + Q2C + Q3A + Q3C + Q4C);
};

let VigorosaDuracaoTotal = (Q1G: number, Q2D: number, Q3F: number, Q4F: number): number => {
    return (Q1G + Q2D + Q3F + Q4F);
};

let VigorosaFrequenciaTotal = (Q1F: number, Q2C: number, Q3E: number, Q4E: number): number => {
    return (Q1F + Q2C + Q3E + Q4E);
};

export const resp_caminhada_duracao_total = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
        ...
        console.log('Resposta Caminhada Duração Total:', resp_caminhada_duracao_total);
...
export const resp_caminhada_frequencia_total = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
        const respostas = await db('Responde').select('fk_Usuario_id_usuario', 'fk_Questao_id_questao', 'respostas_abertas');
        const usuarios: { [id: number]: any } = {};
        respostas.forEach((resposta: any) => {
        ...
        console.log('Resposta Total Caminhada Frequencia Total:', resp_caminhada_frequencia_total);
        return reply.send(resp_caminhada_frequencia_total);
    } catch (error) {
        console.error('Erro no cálculo do total da Caminhada Frequencia Total!', error);
        return reply.status(500).send({ message: 'Erro ao buscar dados!' });
    }
};

export const resp_moderada_duracao_total = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
        const respostas = await db('Responde').select('fk_Usuario_id_usuario', 'fk_Questao_id_questao', 'respostas_abertas');
        ...
        respostas.forEach((resposta: any) => {
            if (!usuarios[resposta.fk_Usuario_id_usuario]) {
                usuarios[resposta.fk_Usuario_id_usuario] = { Q1E: 0, Q2D: 0, Q3B: 0, Q3D: 0, Q4D: 0 };
            }
            PegandoQuestoes(resposta.fk_Questao_id_questao, resposta.respostas_abertas, usuarios[resposta.fk_Usuario_id_usuario]);
        });
        const resp_moderada_duracao_total = Object.keys(usuarios).map((id) => {
            ...
            return {
                id: Number(id),
                totalCaminhada: ModeradaDuracaoTotal(user.Q1E, user.Q2D, user.Q3B, user.Q3D, user.Q4D), // Total de caminhada vigorosa
            };
        });

        console.log('Resposta Moderada Duracao Total:', resp_moderada_duracao_total);
...
export const resp_frequencia_moderada_total = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
        const respostas = await db('Responde').select('fk_Usuario_id_usuario', 'fk_Questao_id_questao', 'respostas_abertas');
        const usuarios: { [id: number]: any } = {};
        respostas.forEach((resposta: any) => {
            if (!usuarios[resposta.fk_Usuario_id_usuario]) {
                usuarios[resposta.fk_Usuario_id_usuario] = { Q1D: 0, Q2C: 0, Q3A: 0, Q3C: 0, Q4C: 0 };
            }
            PegandoQuestoes(resposta.fk_Questao_id_questao, resposta.respostas_abertas, usuarios[resposta.fk_Usuario_id_usuario]);
        });
        const resp_frequencia_moderada_total = Object.keys(usuarios).map((id) => {
            ...
            return {
                id: Number(id),
                totalCaminhada: ModeradaFrequenciaTotal(user.Q1D, user.Q2C, user.Q3A, user.Q3C, user.Q4C), // Total de caminhada moderada
            };
        });

        console.log('Resposta da Frequencia Moderada Total:', resp_frequencia_moderada_total);
...
export const resp_vigorosa_duracao_total = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
        const respostas = await db('Responde').select('fk_Usuario_id_usuario', 'fk_Questao_id_questao', 'respostas_abertas');
        ...
        respostas.forEach((resposta: any) => {
            if (!usuarios[resposta.fk_Usuario_id_usuario]) {
                usuarios[resposta.fk_Usuario_id_usuario] = { Q1G: 0, Q2D: 0, Q3F: 0, Q4F: 0 };
            }
            PegandoQuestoes(resposta.fk_Questao_id_questao, resposta.respostas_abertas, usuarios[resposta.fk_Usuario_id_usuario]);
        });
        const resp_vigorosa_duracao_total = Object.keys(usuarios).map((id) => {
            ...
            return {
                id: Number(id),
                totalCaminhada: VigorosaDuracaoTotal(user.Q1G, user.Q2D, user.Q3F, user.Q4F), // Total de caminhada vigorosa
            };
        });

        console.log('Resposta Moderada Duracao Total:', resp_vigorosa_duracao_total);
...
export const resp_frequencia_vigorosa_total = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
        const respostas = await db('Responde').select('fk_Usuario_id_usuario', 'fk_Questao_id_questao', 'respostas_abertas');
        const usuarios: { [id: number]: any } = {};
        respostas.forEach((resposta: any) => {
            if (!usuarios[resposta.fk_Usuario_id_usuario]) {
                usuarios[resposta.fk_Usuario_id_usuario] = { Q1F: 0, Q2C: 0, Q3E: 0, Q4E: 0 };
            }
            PegandoQuestoes(resposta.fk_Questao_id_questao, resposta.respostas_abertas, usuarios[resposta.fk_Usuario_id_usuario]);
        });
        const resp_frequencia_vigorosa_total = Object.keys(usuarios).map((id) => {
            ...
            return {
                id: Number(id),
                totalCaminhada: VigorosaFrequenciaTotal(user.Q1F, user.Q2C, user.Q3E, user.Q4E), // Total de caminhada moderada
            };
        });

        console.log('Resposta da Frequencia Vigorosa Total:', resp_frequencia_vigorosa_total);
...
let CaminhadaDuracaoMedia = (Q1C: number, Q2F: number, Q4B: number, Q1B: number, Q2E: number, Q4A: number): number => {
...
export const resp_caminhada_duracao_media = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
        const respostas = await db('Responde').select('fk_Usuario_id_usuario', 'fk_Questao_id_questao', 'respostas_abertas');
        ...
        respostas.forEach((resposta: any) => {
            if (!usuarios[resposta.fk_Usuario_id_usuario]) {
                usuarios[resposta.fk_Usuario_id_usuario] = { Q1C: 0, Q2F: 0, Q4B: 0, Q1B: 0, Q2E: 0, Q4A: 0 };
            }
            PegandoQuestoes(resposta.fk_Questao_id_questao, resposta.respostas_abertas, usuarios[resposta.fk_Usuario_id_usuario]);
        });
        ...
        const resp_caminhada_duracao_media = Object.keys(usuarios).map((id) => {
            const user = usuarios[Number(id)];
            const media = CaminhadaDuracaoMedia(user.Q1C, user.Q2F, user.Q4B, user.Q1B, user.Q2E, user.Q4A); // Calcula a média
            return {
                id: Number(id),
        ...
        console.log('Resposta Caminhada Duração Média:', resp_caminhada_duracao_media);
...
let DuracaoMediaModerada = (Q1E: number, Q2D: number, Q3B: number, Q3D: number, Q4D: number, Q1D: number, Q2C: number, Q3A: number, Q3C: number, Q4C: number): number => {
...
export const resp_duracao_media_moderada = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
        const respostas = await db('Responde').select('fk_Usuario_id_usuario', 'fk_Questao_id_questao', 'respostas_abertas');
        ...
        respostas.forEach((resposta: any) => {
            if (!usuarios[resposta.fk_Usuario_id_usuario]) {
                usuarios[resposta.fk_Usuario_id_usuario] = { Q1E: 0, Q2D: 0, Q3B: 0, Q3D: 0, Q4D: 0, Q1D: 0, Q2C: 0, Q3A: 0, Q3C: 0, Q4C: 0 };
            }
            PegandoQuestoes(resposta.fk_Questao_id_questao, resposta.respostas_abertas, usuarios[resposta.fk_Usuario_id_usuario]);
        });
        ...
        const resp_duracao_media_moderada = Object.keys(usuarios).map((id) => {
            const user = usuarios[Number(id)];
            const media = DuracaoMediaModerada(user.Q1E, user.Q2D, user.Q3B, user.Q3D, user.Q4D, user.Q1D, user.Q2C, user.Q3A, user.Q3C, user.Q4C); // Calcula a média
            return {
                id: Number(id),
        ...
        console.log('Resposta Moderada Duração Média:', resp_duracao_media_moderada);
...
let DuracaoMediaVigorosa = (Q1G: number, Q2D: number, Q3F: number, Q4F: number, Q1F: number, Q2C: number, Q3E: number, Q4E: number): number => {
...
export const resp_duracao_media_vigorosa = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
        const respostas = await db('Responde').select('fk_Usuario_id_usuario', 'fk_Questao_id_questao', 'respostas_abertas');
        ...
        respostas.forEach((resposta: any) => {
            if (!usuarios[resposta.fk_Usuario_id_usuario]) {
                usuarios[resposta.fk_Usuario_id_usuario] = { Q1G: 0, Q2D: 0, Q3F: 0, Q4F: 0, Q1F: 0, Q2C: 0, Q3E: 0, Q4E: 0 };
            }
            PegandoQuestoes(resposta.fk_Questao_id_questao, resposta.respostas_abertas, usuarios[resposta.fk_Usuario_id_usuario]);
        });
        ...
        const resp_duracao_media_vigorosa = Object.keys(usuarios).map((id) => {
            const user = usuarios[Number(id)];
            const media = DuracaoMediaVigorosa(user.Q1G, user.Q2D, user.Q3F, user.Q4F, user.Q1F, user.Q2C, user.Q3E, user.Q4E); // Calcula a média
            return {
                id: Number(id),
        ...
        console.log('Resposta Vigorosa Duração Média:', resp_duracao_media_vigorosa);
...
let FrequenciaMediaModCam = (Q1B: number, Q2E: number, Q4A: number, Q1D: number, Q2C: number, Q3A: number, Q3C: number, Q4C: number): number => {
...
export const resp_freq_mod_cam = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
        const respostas = await db('Responde').select('fk_Usuario_id_usuario', 'fk_Questao_id_questao', 'respostas_abertas');
        ...
        const resp_freq_mod_cam = Object.keys(usuarios).map((id) => {
            const user = usuarios[Number(id)];
            const media = FrequenciaMediaModCam(user.Q1B, user.Q2E, user.Q4A, user.Q1D, user.Q2C, user.Q3A, user.Q3C, user.Q4C); // Calcula a média
            return {
                id: Number(id),
        ...
        console.log('Resposta Frequencia Media Moderada + Caminhada:', resp_freq_mod_cam);
        return reply.send(resp_freq_mod_cam);
    } catch (error) {
        console.error('Erro no cálculo da Frequencia Media Moderada + Caminhada!', error);
        return reply.status(500).send({ message: 'Erro ao buscar dados!' });
    }
};
...
export const resp_dur_mod_cam = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
        const respostas = await db('Responde').select('fk_Usuario_id_usuario', 'fk_Questao_id_questao', 'respostas_abertas');
        ...
        const resp_dur_mod_cam = Object.keys(usuarios).map((id) => {
            const user = usuarios[Number(id)];
            const mediaDCaminhada = CaminhadaDuracaoMedia(user.Q1C, user.Q2F, user.Q4B, user.Q1B, user.Q2E, user.Q4A);
            const duracaoTotalModerada = ModeradaDuracaoTotal(user.Q1E, user.Q2D, user.Q3B, user.Q3D, user.Q4D);
            const duracaoTotalCaminhada = CaminhadaDuracaoTotal(user.Q1C, user.Q2F, user.Q4B);

            if (mediaDCaminhada > 0) {
                media = (duracaoTotalCaminhada + duracaoTotalModerada) / mediaDCaminhada;
            }

            return {
                id: Number(id),
                mediaDuracaoMediaModCam: Number(media.toFixed(2)), // Formata a média com 2 casas decimais
            };
        });

        console.log('Resposta Duração Media Moderada + Caminhada:', resp_dur_mod_cam);
        return reply.send(resp_freq_mod_cam);
    } catch (error) {
        console.error('Erro no cálculo da Duração Media Moderada + Caminhada!', error);
        return reply.status(500).send({ message: 'Erro ao buscar dados!' });
    }
};
...
export const classificacao = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
        const respostas = await db('Responde').select('fk_Usuario_id_usuario', 'fk_Questao_id_questao', 'respostas_abertas');
        ...
        const classificacao = Object.keys(usuarios).map((id) => {
            ...
            const AT = ModeradaDuracaoTotal(user.Q1E, user.Q2D, user.Q3B, user.Q3D, user.Q4D);
            const AR = CaminhadaDuracaoTotal(user.Q1C, user.Q2F, user.Q4B);
            const AW = VigorosaFrequenciaTotal(user.Q1F, user.Q2C, user.Q3E, user.Q4E);
            const AX = DuracaoMediaVigorosa(user.Q1G, user.Q2D, user.Q3F, user.Q4F, user.Q1F, user.Q2C, user.Q3E, user.Q4E);
            const AU = ModeradaFrequenciaTotal(user.Q1D, user.Q2C, user.Q3A, user.Q3C, user.Q4C);
            const AS = CaminhadaFrequenciaTotal(user.Q1B, user.Q2E, user.Q4A);
            const AY = DuracaoMediaVigorosa(user.Q1G, user.Q2D, user.Q3F, user.Q4F, user.Q1F, user.Q2C, user.Q3E, user.Q4E);
            const AZ = CaminhadaDuracaoMedia(user.Q1C, user.Q2F, user.Q4B, user.Q1B, user.Q2E, user.Q4A);
            const BA = FrequenciaMediaModCam(user.Q1B, user.Q2E, user.Q4A, user.Q1D, user.Q2C, user.Q3A, user.Q3C, user.Q4C);
            const BB = media;
